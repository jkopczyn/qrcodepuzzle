...
#   (s11 -> x1) AND        # rider: s11 *must* match x1
#   (x4 -> ~s33) AND       # max_3 constraint itself, if last var is true total must be <3 before it
#   (~s12 AND ~s13) AND    # impossible to have >1 true in 1 var
#   (s43) AND              # rider: the last variable must have exactly the right counter
...
#   (s21 -> (x2 OR s11)) AND            # rider: and s21 is only true if a prereq is met
#       (       # iterate for s_2j from 1 < j < N
#       ((x2 AND s11) -> s22) AND (s12 -> s22) AND  # counter stands at 2 if it was 2 or was 1 and +1
#       (s22 -> (x2 OR s12)) AND (s22 -> (s11 OR s12))  # rider: if and only if
